...
import { getCustomerByIdentifierWithDep } from '../core/interactors';
import { StatusCodes } from 'http-status-codes';
import { idBadRequestMessage } from 'bank-utils/api/v1/helpers';

export const getCustomerByIdentifierController = async ({ params }: Request, response: Response): Promise<void> => {
  ...
  // If request is valid, call the interactor
    ...
    const { success, ...rest } = await getCustomerByIdentifierWithDep(intIdentifier);
