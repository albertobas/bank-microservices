...
import { getLoanByIdWithDep } from '../core/interactors';
import { StatusCodes } from 'http-status-codes';
import { identifierBadRequestMessage } from 'bank-utils/api/v1/helpers';

export const getLoanByIdController = async ({ params }: Request, response: Response): Promise<void> => {
  ...
  // If request is valid, call the interactor
    ...
    const { success, ...rest } = await getLoanByIdWithDep(intIdentifier);
